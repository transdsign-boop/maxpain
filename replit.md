...
- **Position Architecture** (Updated Oct 15, 2025): Database has been fully consolidated to single-source-of-truth positions. Each realized P&L event is represented by ONE position with real exchange fills. **Legacy P&L sync permanently disabled** - the reconciliation loop no longer calls `syncCompletedTrades()`. All P&L data is now fetched on-demand from exchange API, eliminating duplicate synthetic fills. Manual cleanup completed to remove all `sync-pnl-*` fills. **Completed trades UI updated** - frontend now displays positions with real fills from actual trading (endpoint filters OUT synthetic fills). **Fills attachment bug fixed** (Oct 15, 2025): `/api/strategies/:strategyId/positions/closed` endpoint now properly attaches fills array to each position (previously fetched fills but didn't include them in response). **Session ID resolution added** (Oct 15, 2025): Endpoint now accepts both strategy IDs and session IDs - if a session ID is provided, it automatically resolves the associated strategy, ensuring "No completed trades" errors are eliminated. **Legacy layer tracking removed** (Oct 15, 2025): `position_layers` table fully removed from schema and codebase after migrating to position-level TP/SL managed by OrderProtectionService. **P&L enrichment implemented (Production-Ready)** (Oct 15, 2025): `/api/strategies/:strategyId/positions/closed` endpoint dynamically enriches database positions with actual realized P&L using deterministic orderId-based matching from `/fapi/v1/userTrades` API. Each trade contains `realizedPnl` field which is summed for matching trades (symbol + time window + side/positionSide). Database positions retain layer/DCA tracking while exchange provides real money values. **Fully deterministic in normal operation**: Trades matched by symbol, position lifetime, and side direction - no timestamp heuristics. Edge cases may exist with overlapping hedge positions or trades outside the query window.
