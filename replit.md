...
- **Singleton Session Model (2025-10-01):** Simplified trading architecture to use a single persistent session with strategy parameters as adjustable settings (not separate entities). Per user request, eliminated multi-session complexity. System now: (1) maintains ONE continuous trading session that never ends, (2) updates balance in-place when changed (cancels pending orders first), (3) preserves all historical data in same session, (4) strategy engine loads single default strategy per user via getOrCreateDefaultStrategy(), (5) frontend UI simplified from "Strategy Management" to "Trading Settings" with delete functionality removed. Backend enforces singleton pattern - only one active strategy and one active session per user
- **Production-Ready Balance Reset System (2025-10-01):** [DEPRECATED - replaced by singleton session model] Previous multi-session approach where balance changes created new sessions. System now updates balance in-place within the single persistent session
- **Configurable Liquidation Lookback Window (2025-10-01):** Added user-configurable lookback window (1-24 hours) for percentile-based trade signals, replacing the fixed 60-second window. Users can now select from preset intervals (1, 2, 4, 6, 8, 12, 18, or 24 hours) to compare liquidations against recent historical data. This allows for more flexible trading strategies - shorter windows for reactive trading, longer windows for trend analysis. Single liquidations are now evaluated (removed 2-liquidation minimum). Strategy engine correctly uses configurable lookback period: `liquidationLookbackHours * 3600` seconds
...
- **Fixed Final Race Condition in Liquidation Deduplication (2025-10-01):** Resolved critical race condition where duplicates arriving 2-17ms apart both bypassed the processingQueue check. Root cause: logic checked processingQueue but didn't SET it until 4 steps later, allowing concurrent duplicates to pass through before either locked the signature. Fixed by moving processingQueue.set() immediately after the check, creating atomic lock. Deleted 8 duplicate liquidations post-fix. Zero duplicates in 5-minute verification period. System now correctly serializes all Aster DEX duplicate message broadcasts
