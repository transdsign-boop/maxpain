...
- **Percentile Threshold**: Entry and layer decisions use percentile rank calculated from ALL historical liquidations for each symbol (matching UI badge calculation). 60% threshold = only enter liquidations at 60th percentile or higher (top 40%). Backend queries database for up to 10,000 historical liquidations per symbol (same as frontend) to ensure accurate percentile calculations that match UI badges. Both `shouldEnterPosition()` and `shouldAddLayer()` use `storage.getLiquidationsBySymbol()` for consistent data source.
