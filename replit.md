...
- **Hedge Mode Orders (CRITICAL)**: All protective orders MUST include `positionSide` parameter (LONG/SHORT) for hedge mode compatibility. When `positionSide` is set, `reduceOnly` is automatically implied and must NOT be sent to exchange (causes "Parameter 'reduceOnly' sent when not required" error).
