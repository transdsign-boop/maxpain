...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Manual endpoint to find and fix stuck positions
  app.post('/api/positions/fix-stuck', async (req, res) => {
    try {
      const { symbol, realizedPnl, closedAt } = req.body;
      
      if (!symbol || realizedPnl === undefined || !closedAt) {
        return res.status(400).json({ 
          error: 'symbol, realizedPnl, and closedAt are required' 
        });
      }
      
      console.log(`🔧 Attempting to fix stuck position for ${symbol}`);
      
      // Get all strategies to find active sessions
      const strategies = await storage.getStrategiesByUser(DEFAULT_USER_ID);
      const activeStrategy = strategies.find(s => s.isActive);
      ...
      // Find the stuck position (marked as isOpen=true)
      const openPositions = await storage.getOpenPositions(session.id);
      const stuckPosition = openPositions.find(p => p.symbol === symbol);
      
      if (!stuckPosition) {
        return res.status(404).json({ 
          error: `No open position found for ${symbol}. It may already be closed.` 
        });
      }
      
      console.log(`✅ Found stuck position: ${stuckPosition.id} for ${symbol}`);
      console.log(`   Entry: $${stuckPosition.avgEntryPrice}, Quantity: ${stuckPosition.totalQuantity}`);
      console.log(`   Closing with realizedPnl: $${realizedPnl} at ${closedAt}`);
      
      // Calculate percentage P&L for display
      const avgEntryPrice = parseFloat(stuckPosition.avgEntryPrice);
      const totalCost = parseFloat(stuckPosition.totalCost);
      const leverage = (stuckPosition as any).leverage || 1;
      const notionalValue = totalCost * leverage;
      const pnlPercent = (realizedPnl / notionalValue) * 100;
      
      // Close the position with the provided data
      ...
      console.log(`✅ Successfully fixed stuck position ${symbol}`);
      console.log(`   Realized P&L: $${realizedPnl.toFixed(2)} (${pnlPercent.toFixed(2)}%)`);
      console.log(`   Session balance updated: $${oldBalance.toFixed(2)} → $${newBalance.toFixed(2)}`);
      
      res.json({
        success: true,
        message: `Position ${symbol} successfully fixed and closed`,
        position: {
          id: stuckPosition.id,
          symbol: stuckPosition.symbol,
      ...
      console.error('Error fixing stuck position:', error);
      res.status(500).json({ error: 'Failed to fix stuck position' });
    }
  });
