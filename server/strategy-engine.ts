...
export class StrategyEngine extends EventEmitter {
            ...
            try {
              ...
              const isBotOrder = order.clientOrderId &&
                (order.clientOrderId.startsWith('bot-') ||
                 ...
                 order.clientOrderId.startsWith('sync-'));
              ...
              // If this is NOT a bot order and we have a position, save it as a manual fill
              if (!isBotOrder && position) {
                console.log(`📝 Manual trade detected: ${order.symbol} ${order.side} qty=${order.lastFilledQuantity} @ ${order.lastFilledPrice}`);

                try {
                  // Check if this fill already exists (using exchange tradeId)
                  const existingFills = await storage.getFillsByOrder(`manual-${order.tradeId}`);

                  if (existingFills.length === 0) {
                    // Save manual fill to database
                    const fillQty = parseFloat(order.lastFilledQuantity || '0');
                    const fillPrice = parseFloat(order.lastFilledPrice || order.averagePrice || '0');
                    const fillValue = fillQty * fillPrice;
                    const commission = parseFloat(order.commission || '0');

                    await storage.applyFill({
                      orderId: `manual-${order.tradeId}`, // Use tradeId to prevent duplicates
                      ...
                      layerNumber: position.layersFilled || 1,
                      source: 'manual', // Tag as manual trade
                      filledAt: new Date(order.orderTradeTime || Date.now()),
                    });

                    console.log(`✅ Manual fill saved: ${order.symbol} ${order.side} ${fillQty} @ ${fillPrice} (fee: ${commission})`);
                  }
                } catch (fillError: any) {
                  // Ignore unique constraint violations (duplicate detection)
                  if (fillError?.code !== '23505') {
                    console.error('❌ Failed to save manual fill:', fillError);
                  }
                }
              }
  ...
  private async evaluateStrategySignal(strategy: Strategy, liquidation: Liquidation) {
    ...
    try {
      ...
      if (existingPosition && existingPosition.isOpen) {
        ...
        if (shouldEnter) {
          // CRITICAL: Set GLOBAL cooldown BEFORE placing order to prevent race conditions
          // This ensures multiple rapid liquidations don't all create entry orders
          this.lastFillTime.set(cooldownKey, Date.now());
          console.log(`🔒 GLOBAL RESERVATION (ENTRY): ${liquidation.symbol} ${positionSide} - cooldown set BEFORE order placement`);
          ...
          if (positionId) {
            ...
            // KEEP cooldown active (already set at line 791) to prevent rapid retry attempts
            this.processedLiquidations.delete(liquidation.id);
            console.log(`🔓 Entry failed - cleared liquidation ID for ${liquidation.symbol} ${positionSide} to allow retry (20s cooldown remains active)`);
          }
        } else {
  ...
  private async fillPaperOrder(order: Order, fillPrice: number, fillQuantity: number, tradeType: 'entry' | 'layer' | 'stop_loss' | 'take_profit' = 'entry') {
    ...
    await storage.applyFill({
      ...
      source: 'bot', // Bot-initiated paper trade
    });
  ...
  private async fillLiveOrder(order: Order, actualFillPrice: number, actualFillQty: number, actualCommission: number, actualTimestamp?: number) {
    ...
    await storage.applyFill({
      ...
      layerNumber: order.layerNumber,
      source: 'bot', // Bot-initiated live trade
  ...
  private async closePosition(
    ...
    try {
      ...
      await storage.applyFill({
        ...
        source: 'bot', // Bot-initiated exit (TP/SL)
