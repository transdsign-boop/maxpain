...
export default function LiquidationAnalytics() {
  ...
  useEffect(() => {
    if (availableAssets && availableAssets.length > 0 && !selectedAsset) {
    ...
    <Card data-testid="card-liquidation-analytics">
      <CardHeader>
        ...
        <p className="text-sm text-muted-foreground">
          Analyze liquidation percentiles and statistics for specific assets
        </p>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        {!selectedAsset && !assetsLoading && availableAssets && availableAssets.length > 0 && (
          <div className="text-center p-6 bg-muted/50 border rounded-lg" data-testid="select-asset-prompt">
            ...
            <Button 
              onClick={() => availableAssets.length > 0 && setSelectedAsset(availableAssets[0].symbol)}
              data-testid="button-auto-select"
            >
              Analyze {availableAssets[0]?.symbol}
            </Button>
          </div>
        )}
      </CardContent>
